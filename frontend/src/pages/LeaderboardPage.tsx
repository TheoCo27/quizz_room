  ...
  return (
    <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-8 px-6 py-10 md:px-10">
      <CyberPanel className="rounded-4xl p-6">
        <p className="cyber-eyebrow">Classement Global</p>
        <h1 className="mt-2 cyber-title text-2xl text-text">
          Top Joueurs par Victoires
        </h1>
        <p className="mt-3 text-sm text-text-muted">
          Decouvre les meilleurs joueurs de la plateforme classes par le nombre total de victoires.
        </p>
      </CyberPanel>

      <CyberCard className="rounded-4xl p-6">
        {isLoading ? (
          <p className="text-center text-text-muted">Chargement du classement...</p>
        ) : leaderboard.length === 0 ? (
          <p className="text-center text-text-muted">Aucune donnee disponible pour le moment.</p>
        ) : (
            ...
            <table className="w-full text-left text-sm text-text">
              <thead>
                <tr className="border-b border-white/10 text-xs uppercase tracking-[0.1em] text-text-muted">
                  ...
                  <th className="px-4 py-3 font-medium text-right">Victoires</th>
                </tr>
              </thead>
              <tbody>
